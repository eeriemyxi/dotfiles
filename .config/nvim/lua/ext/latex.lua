...
function latex_topng(fnames, size)
    ...
    for k, fname in pairs(fnames) do
        jobid = vim.fn.jobstart(
            "latex  --interaction=nonstopmode --output-dir=/tmp --output-format=dvi LATEXFILE"
                .. fname,
            {
                cwd = "/tmp",
...
function insertpngs(eqpos, fnames, size)
    ...
    for i = 1, counttable(eqpos), 2 do
        source = "/tmp/LATEXFILE"
            .. fnames[findex]
            .. ".png"
        buf = vim.api.nvim_get_current_buf()
        holi = require("hologram.image")
        table.insert(imgs, holi:new(source, {}))
        imgs[findex]:display(
            tonumber(eqpos[i + 1][1]),
            0,
            buf,
            {}
        )
